...
public enum Feature {
	...
	public static Feature randomExpertise(Actor actor) {
		...
		Proficiency candidate = list.get(0);
		list.remove(0);
		Feature expertise = matchExpertiseToSkill(candidate);
		while (features.contains(expertise)) {
			candidate = list.get(0);
			list.remove(0);
			expertise = matchExpertiseToSkill(candidate);
		}
	...
	public static HashSet<Feature> bard(Actor actor) {
		...
		if (level == 1) {
			...
			// BARD COLLEGE
			list.add(randomExpertise(actor));
			list.add(randomExpertise(actor));
